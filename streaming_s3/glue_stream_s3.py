...
def processBatch(data_frame, batchId):
    if data_frame.count() > 0:
        ...
        obj_data_frame = KinesisStream.select_fields(paths=["obj"]).toDF()
        #obj_data_frame.show()

        if obj_data_frame.count() > 0:
            csvPaths = list(obj_data_frame.select("obj").toPandas()["obj"])
            print("csvPaths: " + ",".join(csvPaths))
            ...
            S3bucket = glueContext.create_dynamic_frame.from_options(
                ...
                connection_type="s3",
                format="csv",
                connection_options={"paths": csvPaths, "recurse": True},
